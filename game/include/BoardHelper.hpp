...
public:
     ...
     */
    static bool isReversible(const std::vector<std::vector<char> > &board,
     ...
     */
    static bool isValidMove(const std::vector<std::vector<char> > &board,
                            const Position &pos, char player);

    /**
     * @brief Reverses the opponent's pieces according to Othello rules after a
     * valid move.
     * @param board 2D char vector representing the current state of the board. It
     * will be updated with the new board state after reversing pieces.
     * @param pos The position at which the player has just placed a piece.
     * @param player The character representing the current player.
     */
    static void reversePieces(std::vector<std::vector<char> > &board,
                              const Position &pos, char player);

    /**
     * @brief Places the player's piece at the specified position and reverses any
     * opponent pieces accordingly.
     * @param board 2D char vector representing the current state of the board. It
     * will be updated with the new board state after reversing pieces.
     * @param pos The position at which the player has just placed a piece.
     * @param player The character representing the current player.
     */
    static void playMove(std::vector<std::vector<char> > &board,
                         const Position &pos, char player);
     ...
     */
    static std::vector<Position> getAllPossibleMoves(
    ...
    /**
     * @brief Switches the player if the current player has no valid moves. If no
     * one can play, returns false.
     * @param board 2D char vector representing the board.
     * @param player The current player.
     * @return true if a player can play, false otherwise.
     */
    static bool switchPlayer(const std::vector<std::vector<char> > &board,
                             char &player);
     ...
     */
    static int countPlayerPieces(const std::vector<std::vector<char> > &board,
                                 char player);
     ...
     */
    static int countTotalPieces(const std::vector<std::vector<char> > &board);
     ...
     */
    static std::vector<std::vector<char> > getBoardAfterMove(
...
private:
     ...
     */
    static std::vector<Position> getReversePoints(
        std::vector<std::vector<char> > &oBoard, char player, const Position &pos);

    /** @brief Helper function to check in the up direction. */
    static std::vector<Position> checkUp(std::vector<std::vector<char> > &board,
                                         char player, const Position &pos);

    /** @brief Helper function to check in the up-right direction. */
    static std::vector<Position> checkUpRight(
        std::vector<std::vector<char> > &board, char player, const Position &pos);

    /** @brief Helper function to check in the right direction. */
    static std::vector<Position> checkRight(std::vector<std::vector<char> > &board,
                                            char player, const Position &pos);

    /** @brief Helper function to check in the down-right direction. */
    static std::vector<Position> checkDownRight(
        std::vector<std::vector<char> > &board, char player, const Position &pos);

    /** @brief Helper function to check in the down direction. */
    static std::vector<Position> checkDown(std::vector<std::vector<char> > &board,
                                           char player, const Position &pos);

    /** @brief Helper function to check in the down-left direction. */
    static std::vector<Position> checkDownLeft(
        std::vector<std::vector<char> > &board, char player, const Position &pos);

    /** @brief Helper function to check in the left direction. */
    static std::vector<Position> checkLeft(std::vector<std::vector<char> > &board,
                                           char player, const Position &pos);

    /** @brief Helper function to check in the up-left direction. */
    static std::vector<Position> checkUpLeft(
        std::vector<std::vector<char> > &board, char player, const Position &pos);
};
